Suurepärane! Liigume edasi PowerShelli programmeerimise "mootori" juurde. Kui seni kasutasid käske ühekaupa, siis see peatükk õpetab sind koodi automatiseerima.

---

## Peatükk 4: Muutujad, massiivid ja tsüklid

See peatükk on PowerShelli süda. Siin õpime andmeid mälus hoidma (muutujad), neid grupeerima (massiivid) ja korduvaid tegevusi automaatselt sooritama (tsüklid).

### 1. Muutujad (Variables)

Muutuja on nagu "mälukarp", kuhu saad andmed hoiule panna. PowerShellis algavad kõik muutujad **$** märgiga.
...
Tsükkel on skripti osa, mis kordab tegevust seni, kuni nimekiri saab otsa või tingimus on täidetud.

#### A. `foreach` tsükkel (Kõige olulisem!)

Seda kasutatakse massiivi (nimekirja) läbimiseks. See tähendab: "Võta see nimekiri ja tee iga elemendiga midagi".
...
Kasutatakse siis, kui tead täpselt, mitu korda soovid midagi teha (nt loendada 1-st 10-ni).
...
while ($number -lt 5) {
    ...
    $number++ # Suurendab numbrit ühe võrra
}
...
1. **Harjutus: Puuviljad.** Loo massiiv viie puuvilja nimega. Kirjuta `foreach` tsükkel, mis ütleb iga puuvilja kohta: "Mulle maitseb [puuvilja nimi]".
2. **Harjutus: Arvude ruudud.** Kasuta `for` tsüklit, et kuvada arvud 1-st 10-ni ning nende ruudud (arv korrutatud iseendaga).
3. **Harjutus: Failide mass-loomine.** Loo massiiv kolme nimega: `test1`, `test2`, `test3`. Kirjuta `foreach` tsükkel, mis loob igale nimele vastava `.txt` faili sinu töölauale.
4. **Harjutus: Teenuste kontroll.** Salvesta muutujasse kõik seisvad teenused (`Get-Service | Where-Object {$_.Status -eq 'Stopped'}`). Kasuta `foreach` tsüklit, et kuvada ekraanile nende teenuste nimed punase värviga.
...
1. **Andmete ettevalmistus:** Loo massiiv `$tootajad`, kuhu märgi vähemalt 5 nime (nt "Kadi", "Mati", "Tiina", "Siim", "Jaan").
2. **Struktuuri loomine:** Kirjuta `foreach` tsükkel, mis teeb iga töötaja jaoks järgmist:
* Loo kaust asukohta `C:\Temp\Kasutajad\[TöötajaNimi]`.
* Loo selle kausta sisse fail `tervitus.txt`.
* Kirjuta faili tervitustekst: "Tere, [TöötajaNimi]! Sinu konto on loodud $(Get-Date)".


3. **Väljund:** Pärast tsüklit peab skript väljastama teate: "Töö lõpetatud. Kokku loodi [massiivi pikkus] kasutaja keskkonda."

**Esitamiseks:**

* Esita skripti kood (`.ps1` tekstina).
* Selgita, mida tähistab tsükli päises `($tootaja in $tootajad)` esimene osa (muutuja `$tootaja`).
* Esita screenshot `C:\Temp\Kasutajad` kaustast, kus on näha loodud kataloogid.

---

**Kas liigume edasi Peatükk 5 juurde, kus õpime skripte ja logimist (kuidas vigu püüda ja tegevusi faili kirja panna)?**
